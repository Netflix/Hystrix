...
public class HystrixCollapserTest {
    ...
    class MyCollapser extends HystrixCollapser<List<Pair<String, Integer>>, Integer, String> {
        ...
        MyCollapser(String arg, boolean reqCacheEnabled) {
            super(HystrixCollapserKey.Factory.asKey("UNITTEST"), Scope.REQUEST, new RealCollapserTimer(), HystrixCollapserProperties.Setter().withRequestCacheEnabled(reqCacheEnabled), HystrixCollapserMetrics.getInstance(HystrixCollapserKey.Factory.asKey("UNITTEST"), new HystrixPropertiesCollapserDefault(HystrixCollapserKey.Factory.asKey("UNITTEST"), HystrixCollapserProperties.Setter())));
            this.arg = arg;
        }
    ...
    public void testNoRequestCache3() {
        ...
        Iterator<HystrixInvokableInfo<?>> cmdIterator = HystrixRequestLog.getCurrentRequest().getAllExecutedCommands().iterator();
        //1 for A, 1 for B.  Batch contains only unique arguments (no duplicates)
        assertEquals(2, cmdIterator.next().getNumberCollapsed());
        //1 for A, 1 for B.  Batch contains only unique arguments (no duplicates)
        assertEquals(2, cmdIterator.next().getNumberCollapsed());
    }
    ...
    private static class ShortCircuitedCommand extends HystrixCommand<List<String>> {
        ...
        protected ShortCircuitedCommand() {
            super(HystrixCommand.Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("shortCircuitedCommand")).andCommandPropertiesDefaults(HystrixCommandPropertiesTest.getUnitTestPropertiesSetter().withCircuitBreakerForceOpen(true)));
        }
    ...
    private static class FireAndForgetCommand extends HystrixCommand<Void> {
        ...
        protected FireAndForgetCommand(List<Integer> values) {
            super(HystrixCommand.Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("fireAndForgetCommand")).andCommandPropertiesDefaults(HystrixCommandPropertiesTest.getUnitTestPropertiesSetter()));
        }
    ...
    static class TestCollapserTimer implements CollapserTimer {
        ...
        private static class ATask {
            ...
            private synchronized void executeTask() {
                ...
                task.tick();
                // we reset time after each execution
                this.time = 0;
                this.executionCount++;
