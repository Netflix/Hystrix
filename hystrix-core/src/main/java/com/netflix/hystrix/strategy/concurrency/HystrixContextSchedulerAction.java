 ...
 * @ExcludeFromJavadoc
 */
public class HystrixContextSchedulerAction implements Action1<Inner> {
    ...
    public HystrixContextSchedulerAction(Action1<Inner> action) {
        this(HystrixPlugins.getInstance().getConcurrencyStrategy(), action);
    }

    public HystrixContextSchedulerAction(final HystrixConcurrencyStrategy concurrencyStrategy, Action1<Inner> action) {
        this.actual = action;
