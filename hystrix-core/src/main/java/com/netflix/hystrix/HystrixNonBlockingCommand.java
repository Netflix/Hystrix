...
public abstract class HystrixNonBlockingCommand<R> extends AbstractHystrixCommand<R> implements HystrixExecutable<R> {
     ...
     */
    protected HystrixNonBlockingCommand(HystrixCommandGroupKey group) {
        // use 'null' to specify use the default
        this(new Setter(group));
    }

    /**
     * Construct a {@link HystrixNonBlockingCommand} with defined {@link Setter} that allows injecting property and strategy overrides and other optional arguments.
     * <p>
     * NOTE: The {@link HystrixCommandKey} is used to associate a {@link HystrixNonBlockingCommand} with {@link HystrixCircuitBreaker}, {@link HystrixCommandMetrics} and other objects.
     * <p>
     * Do not create multiple {@link HystrixNonBlockingCommand} implementations with the same {@link HystrixCommandKey} but different injected default properties as the first instantiated will win.
     * <p>
     ...
     */
    protected HystrixNonBlockingCommand(Setter setter) {
        // use 'null' to specify use the default
        this(setter.groupKey, setter.commandKey, setter.threadPoolKey, null, null, setter.commandPropertiesDefaults, setter.threadPoolPropertiesDefaults, null, null, null, null, null);
    }

    /**
     * Allow constructing a {@link HystrixNonBlockingCommand} with injection of most aspects of its functionality.
     * <p>
     ...
     */
    private HystrixNonBlockingCommand(HystrixCommandGroupKey group, HystrixCommandKey key, HystrixThreadPoolKey threadPoolKey, HystrixCircuitBreaker circuitBreaker, HystrixThreadPool threadPool,
    ...
    private static String getDefaultNameFromClass(@SuppressWarnings("rawtypes") Class<? extends HystrixNonBlockingCommand> cls) {
    ...
    private static ConcurrentHashMap<Class<? extends HystrixNonBlockingCommand>, String> defaultNameCache = new ConcurrentHashMap<Class<? extends HystrixNonBlockingCommand>, String>();
     ...
     */
    protected abstract Observable<R> run() throws Exception;
    ...
    public Future<R> queue() {
        ...
        return new Future<R>() {
            ...
            protected R performBlockingGetWithTimeout(final ObservableCommand<R> o, final Future<R> f) throws InterruptedException, ExecutionException {
                ...
                HystrixNonBlockingCommand<R> originalCommand = o.getCommand();
                /**
...
//                            } else {
      ...
      //  if (properties.executionIsolationStrategy().get().equals(ExecutionIsolationStrategy.THREAD)) {
            ...
            o = new TimeoutObservable<R>(o, _this, true);
       // }
    ...
    private static class CachedObservableOriginal<R> extends ObservableCommand<R> {
        ...
        final HystrixNonBlockingCommand<R> originalCommand;

        CachedObservableOriginal(final Observable<R> actual, HystrixNonBlockingCommand<R> command) {
    ...
    private static class ObservableCommand<R> extends Observable<R> {
        private final HystrixNonBlockingCommand<R> command;

        ObservableCommand(OnSubscribeFunc<R> func, final HystrixNonBlockingCommand<R> command) {
            super(func);
            this.command = command;
        }

        public HystrixNonBlockingCommand<R> getCommand() {
            return command;
        }

        ObservableCommand(final Observable<R> originalObservable, final HystrixNonBlockingCommand<R> command) {
            super(new OnSubscribeFunc<R>() {
    ...
    private static class CachedObservableResponse<R> extends ObservableCommand<R> {
        ...
        CachedObservableResponse(final CachedObservableOriginal<R> originalObservable, final HystrixNonBlockingCommand<R> commandOfDuplicateCall) {
            super(new OnSubscribeFunc<R>() {
            ...
            super(new OnSubscribeFunc<R>() {
                ...
                public Subscription onSubscribe(final Observer<? super R> observer) {
                    ...
                    TimerListener listener = new TimerListener() {
                        ...
                        @Override
                        public void tick() {
                        	System.out.println("ticked" + System.currentTimeMillis());
                            ...
                            if (originalCommand.isCommandTimedOut.compareAndSet(TimedOutStatus.NOT_EXECUTED, TimedOutStatus.TIMED_OUT)) {
                                ...
                                System.out.println("marked as timed out" + System.currentTimeMillis());
        ...
        if (executionSemaphore.tryAcquire()) {
            try {
                try {
                    ...
                    if(r == null) { // check if actually timed out
                    ...
                    System.out.println("r from exec command" + r);
...
*/
     ...
     * @return R
     */
    private Observable<R> executeCommand() {
        /**
        ...
        try {
            ...
            //Observable<R> response = executionHook.onRunSuccess(this, run());
            ...
            if (isCommandTimedOut.get() == TimedOutStatus.TIMED_OUT) {
                ...
                System.out.println("Error executing HystrixCommand.run() [TimedOut]. Proceeding to fallback logic ..." + e);
                return null;
            } else {
    ...
    private Observable<R> getFallbackWithProtection() {
        ...
        System.out.println("permits in use: " + fallbackSemaphore.getNumberOfPermitsUsed() + " t: " + System.currentTimeMillis());
        // acquire a permit
        if (fallbackSemaphore.tryAcquire()) {
            try {
            	System.out.println("permits in use2: " + fallbackSemaphore.getNumberOfPermitsUsed() + " t: " + System.currentTimeMillis());
             ...
             //   return executionHook.onFallbackSuccess(this, getFallback());
            	...
            	System.out.println("permits in use3: " + fallbackSemaphore.getNumberOfPermitsUsed() + " t: " + System.currentTimeMillis());
                fallbackSemaphore.release();
            }
        } else {
            metrics.markFallbackRejection();
            System.out.println("all fallback permits in use");
     ...
     * @throws HystrixRuntimeException
     */
    private Observable<R> getFallbackOrThrowException(HystrixEventType eventType, FailureType failureType, String message, Exception e) {
        try {
            if (properties.fallbackEnabled().get()) {
                ...
                try {
                    ...
                    Observable<R> fallback = getFallbackWithProtection();
    ...
    public static class UnitTest {
        ...
        public void testObserveOnScheduler() throws Exception {
            ...
            System.out.println("Command Thread: " + commandThread.get().getName());
            System.out.println("Subscribe Thread: " + subscribeThread.get().getName());
...
//            assertTrue(subscribeThread.get().getName().equals("RxScheduledThread"));
            ...
            assertTrue(subscribeThread.get().getName().equals("RxScheduledThread"));
            System.out.println("test observeOn ends");
        }
...
//            HystrixNonBlockingCommand<?>[] executeCommands = HystrixRequestLog.getCurrentRequest().getExecutedCommands().toArray(new HystrixNonBlockingCommand<?>[] {});
//
...
//            assertEquals(0, command.builder.executionHook.threadComplete.get());
       ...
       //     assertEquals(2, HystrixRequestLog.getCurrentRequest().getExecutedCommands().size());
        ...
        private static class TestExecutionHook extends HystrixCommandExecutionHook {
            ...
            @Override
            public <T> Observable<T> onComplete(HystrixNonBlockingCommand<T> commandInstance, Observable<T> response) {
