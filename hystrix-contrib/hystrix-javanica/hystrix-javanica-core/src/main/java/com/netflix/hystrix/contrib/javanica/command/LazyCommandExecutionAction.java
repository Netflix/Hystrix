 ...
 */
public class LazyCommandExecutionAction<T extends MetaHolder, V extends MetaHolder.Builder<V>> implements CommandAction {
    ...
    private T createCopy(T source, ExecutionType executionType) {
        return (T) source.copy().executionType(executionType).build();
    }

    private T createCopy(T source, ExecutionType executionType, Object[] args) {
        return (T) source.copy().executionType(executionType).args(args).build();
    }
}